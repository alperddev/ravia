  ...
  const joinRoom = async (roomId) => {
    ...
    if (snapshot.exists()) {
      ...
      if (bannedUsers[auth.currentUser.uid]) {
        ...
        const roomRefGeneral = doc(firestore, `playRooms/${roomId}`);
        const roomGeneralSnap = await getDoc(roomRefGeneral);
        if (roomGeneralSnap.exists() && roomGeneralSnap.data().Admins === auth.currentUser.uid) {
  ...
  return (
    <SafeAreaView style={styles.View}>
      ...
      <TouchableOpacity style={{paddingBottom:20}}onPress={fetchRooms} >
        {loading ? <ActivityIndicator /> : <FontAwesome name="refresh" size={24} color='white' />}
      </TouchableOpacity>
      <ScrollView>
  {rooms.length === 0 ? (
    <Text style={styles.Text5}>Oda eklemek veya katılmak için lütfen + butonuna tıklayın</Text>
  ) : (
    rooms.map((room, index) => (
      <View key={index} style={{ paddingBottom: 10 }}>
        <TouchableOpacity onPress={() => joinRoom(room.id)} onLongPress={()=>handleLongPress(room.id)}>
