...
export const RoomOptions = ({ modalVisible, setModalVisible, roomId , fetchRooms }) => {

  async function removeRoom() {
    
    const roomRef = doc(firestore, `users/${auth.currentUser.uid}/rooms/${roomId}`);
  ...
  return (
    ...
    >
      ...
      <View style={{backgroundColor:colorPalette.blackL, padding: 20, borderRadius: 10, alignItems: 'center', maxHeight: 400, marginTop: 'auto'}}>
          <TouchableHighlight
            style={styles.Button}
            onPress={() => { removeRoom(); setModalVisible(false); }}
          >
            <Text>Odayi sil</Text>
          </TouchableHighlight>
